...
- 2026-09-22 — Loom-MCP v2.1.0-alpha.29 resolves upstream [#302](https://github.com/CaliLuke/loom-mcp/issues/302) and [#303](https://github.com/CaliLuke/loom-mcp/issues/303). Restored schema defaults, removed the application pagination interceptor, and proved generated validation/defaulting and replayable recovery examples through the official MCP SDK.
...
`metadata.continue` must be a JSON string or absent/null (null means no continuation); reject number/object/array/bool types. Never parse the opaque string or use `internal/service/argo.go`'s trimming `stringPtrValue` helper on it. Explicit MCP limit 0/null is invalid; a zero internal limit means the documented default. Generated input/output `Continue` is optional `*string`, output `HasMore` is required `bool`, and defaulted input `Limit` is `int`. Empty domain/output collections are non-nil slices. Runtime and schema defaults remain 50 for collection lists and 10 for history. Loom-MCP v2.1.0-alpha.29 emits those defaults in schemas and recovery examples, applies them when input omits `limit`, and validates the 1–200 bounds before service dispatch, resolving [loom-mcp#302](https://github.com/CaliLuke/loom-mcp/issues/302) and [loom-mcp#303](https://github.com/CaliLuke/loom-mcp/issues/303). Inspect resulting types in `gen/argo/service.go` and validation/defaulting in `gen/mcp_argo/adapter_server.go` after generation.
