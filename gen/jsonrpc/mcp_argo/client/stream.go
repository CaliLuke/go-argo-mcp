...
// ToolsCallClientStream implements the mcpargo.ToolsCallClientStream interface
...
func (s *ToolsCallClientStream) Recv(ctx context.Context) (*mcpargo.ToolsCallResult, error) {
	s.readLock.Lock()
	defer s.readLock.Unlock()

	var zero *mcpargo.ToolsCallResult
	...
	for {
		...
		case "notification":
			...
			if notification.Method != "mcp_argo/stream.event" {
				continue
			}
...
func (s *ToolsCallClientStream) decodeResult(data json.RawMessage) (*mcpargo.ToolsCallResult, error) {
	resp := &http.Response{
		Body:       io.NopCloser(bytes.NewReader(data)),
		StatusCode: http.StatusOK,
	}
	decoder := s.decoder(resp)
	var result *mcpargo.ToolsCallResult
...
func (s *ToolsCallClientStream) Close() error {
	...
	return nil
} // EventsStreamClientStream implements the mcpargo.EventsStreamClientStream
...
func (s *EventsStreamClientStream) Recv(ctx context.Context) (*mcpargo.EventsStreamResult, error) {
	s.readLock.Lock()
	defer s.readLock.Unlock()

	var zero *mcpargo.EventsStreamResult
...
func (s *EventsStreamClientStream) decodeResult(data json.RawMessage) (*mcpargo.EventsStreamResult, error) {
	resp := &http.Response{
		Body:       io.NopCloser(bytes.NewReader(data)),
		StatusCode: http.StatusOK,
	}
	decoder := s.decoder(resp)
	var result *mcpargo.EventsStreamResult
	if err := decoder.Decode(&result); err != nil {
		return result, err
	}
	return result, nil
