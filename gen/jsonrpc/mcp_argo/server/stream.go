...
// ToolsCallServerStream implements the mcpargo.ToolsCallServerStream interface
...
func (s *ToolsCallServerStream) Send(ctx context.Context, event mcpargo.ToolsCallEvent) error {
	s.mu.Lock()
	if s.closed {
		s.mu.Unlock()
...
func (s *ToolsCallServerStream) SendAndClose(ctx context.Context, event mcpargo.ToolsCallEvent) error {
	s.mu.Lock()
	if s.closed {
		s.mu.Unlock()
...
// EventsStreamServerStream implements the mcpargo.EventsStreamServerStream
...
// Send emits a JSON-RPC notification for one stream event.
func (s *EventsStreamServerStream) Send(ctx context.Context, event mcpargo.EventsStreamEvent) error {
	s.mu.Lock()
	if s.closed {
		s.mu.Unlock()
		return fmt.Errorf("stream closed")
	...
	s.mu.Unlock()
	result, ok := event.(*mcpargo.EventsStreamResult)
	if !ok {
		return fmt.Errorf("unexpected event type: %T", event)
	}
...
// SendAndClose emits the final JSON-RPC response and closes the stream;
// ID-less streams suppress the response.
func (s *EventsStreamServerStream) SendAndClose(ctx context.Context, event mcpargo.EventsStreamEvent) error {
	s.mu.Lock()
	if s.closed {
		s.mu.Unlock()
		return fmt.Errorf("stream already closed")
